        ...
        <form>
          ...
          <v-stepper-items>
            <v-card flat class="mb-12" color="white darken-1" align="center">
              ...
              <InformacoesVaga :isLoaded="isLoaded" :edicao="edicao" :vaga="vaga" @handlePayload="getPayloadInfoVaga" @handleStep="getStep"></InformacoesVaga>
              <template>
  ...
  methods: {
    ...
    async submit(){
      ...
      if(!this.edicao){
        ...
        this.$store.dispatch(actionTypes.UPDATE_VAGA, payload)
        .then(response =>{
          if(response.error != undefined){
            this.notificacoes = response.error
            this.pleaseWaitDialog = false
            ...
            this.pleaseWaitDialog = false
            this.$router.push({ name: 'vagas', params:{updateVagaSucesso: true} })
          }
        })
      }
    },
    async loadDataToEdit(){
      ...
      this.jornada = this.vaga.jornada 
      this.beneficios = this.vaga.beneficio
      this.requisitos = this.vaga.requisito
      this.isLoaded = true
    }
  }
}
