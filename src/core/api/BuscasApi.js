...
const searchAvancadoCurriculos = async ({state}, search) => {
    ...
    //acho que isso aqui seria melhor como um post
    return await axios.get(`
        buscaCurriculosAvancadas/${search.keywords}/${search.escolaridade}/${search.objetivos}/${search.historicoProfissional}/${search.cidade}/${search.nome}/${search.area}?token=${state.auth.token}
    `)
